...
class WhatsApp:
    ...
    def __init__(self, wait, screenshot=None, session=None):
        ...
        if session:
            firefox_options.add_argument("--user-data-dir={}".format(session))
            self.browser = webdriver.Firefox(options=firefox_options, firefox_binary=binary)  # we are using chrome as our webbrowser
        else:
